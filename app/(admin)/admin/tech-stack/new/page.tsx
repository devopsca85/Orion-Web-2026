        ...
        <form action={createTechStack} className="space-y-6">
          <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6 space-y-4">
              ...
              <div className="grid grid-cols-3 gap-2">
                {TECH_CATEGORIES.map((c) => (
                  <label key={c} className="flex items-center gap-2 cursor-pointer text-sm text-slate-700 bg-slate-50 hover:bg-indigo-50 border border-slate-200 hover:border-indigo-300 rounded-lg px-3 py-2 transition-colors">
                    <input type="checkbox" name="category" value={c} className="w-4 h-4 rounded border-slate-300 text-indigo-600 focus:ring-indigo-500" />
                    {c}
                  </label>
                ))}
              </div>
            ...
            <div>
              ...
              <input id="logoUrl" name="logoUrl" type="text" required placeholder="https://cdn.simpleicons.org/react" className={ic} />

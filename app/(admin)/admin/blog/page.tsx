...
export default async function BlogPage({ searchParams }: Props) {
  ...
  return (
    <>
      <AdminTopBar title="Blog Posts" user={session!.user} />
      <div className="p-6">
        {/* Header */}
        <div className="flex items-center justify-between mb-6">
          <p className="text-sm text-slate-500">{total} post{total !== 1 ? 's' : ''} total</p>
          <Link
            href="/admin/blog/new"
                ...
                {posts.length === 0 ? (
                  <tr>
                    <td colSpan={7} className="px-5 py-12 text-center text-slate-400">
                      No posts found. <Link href="/admin/blog/new" className="text-indigo-600 hover:underline">Create one</Link>.
                    </td>
                  </tr>
                ) : (
                  posts.map((post) => {
                    const deleteAction = deleteBlogPost.bind(null, post.slug)
                    return (
                      <tr key={post.slug} className="hover:bg-slate-50">
                        <td className="px-5 py-3 font-medium text-slate-800 max-w-xs">
                          <Link href={`/admin/blog/${post.slug}`} className="hover:text-indigo-600 truncate block">
                            {post.title}
                          </Link>
                        </td>
                        <td className="px-5 py-3 text-slate-500">{post.category}</td>
                        <td className="px-5 py-3 text-slate-500">{post.author.name}</td>
          ...
          {totalPages > 1 && (
            <div className="flex items-center justify-between px-5 py-3 border-t border-slate-100">
              ...
              <div className="flex gap-2">
                {currentPage > 1 && (
                  <Link
                    href={`/admin/blog?status=${status ?? 'all'}&page=${currentPage - 1}`}

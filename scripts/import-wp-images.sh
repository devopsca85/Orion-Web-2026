...
# =============================================================================
# import-wp-images.sh
# Scans WordPress uploads and copies images to the correct Next.js paths.
# Usage: bash scripts/import-wp-images.sh /var/www/html/wp-content/uploads
# =============================================================================

WP_UPLOADS="${1:-/var/www/html/wp-content/uploads}"
...
if [ ! -d "$WP_UPLOADS" ]; then
  echo -e "${RED}WordPress uploads folder not found: $WP_UPLOADS${NC}"
  echo "Usage: bash scripts/import-wp-images.sh /path/to/wp-content/uploads"
  exit 1
fi

echo ""
echo "============================================"
echo " Orion Solutions — WordPress Image Importer"
echo "============================================"
...
# Helper: find best matching image from WP uploads by keyword
find_image() {
  local keyword="$1"
  local extensions="jpg jpeg png webp"
  for ext in $extensions; do
    local result
    result=$(find "$WP_UPLOADS" -iname "*${keyword}*.$ext" 2>/dev/null | head -1)
    if [ -n "$result" ]; then
      echo "$result"
      return
    fi
...
copy_image() {
  ...
  if [ -n "$src" ]; then
    ...
    cp "$src" "$final_dest"
    # Also copy as .jpg if different extension (code references .jpg)
...
# ── PARTNERS (SVG preferred — download official logos) ───────────────────────
echo "[ Partner Logos — checking WordPress first ]"
...
echo ""
echo " Missing images above (✗) need to be copied manually."
echo " Rename the file to match the expected filename and place it"
echo " in the correct subfolder under public/assets/images/"
echo "============================================"
echo ""
